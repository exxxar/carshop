    ...
    <div class="container margin-top25">
        <div class="row">
                    ...
                    <div class="well">
                        <table class="table cart-list">
                            ...
                            @foreach($cart->all() as $item)
                                <tr>
                                    <th class="image">
                                        ...
                                        @if($item->product->img!='/images/products/')
                                            <img src="{{ asset($item->product->img) }}"
                                                 alt="{{ $item->product->title }}">
                                        @else
                                            <img src="{{ url('/images/products/No_Image_Available.jpg') }}"
                                                 alt="{{ $item->product->title }}">
                                        @endif
                                    </th>
                                    <th class="desc">
                                    ...
                                    <th class="amountPrice">{{ $currencySymbol }}
                                        <span>{{ $item->getAmount()->format() }}</span></th>
                                    <th>
                                        <button class="removeItem" data-route="{{ route('cart.remove') }}"
                                                data-product="{{ $item->product->id }}">
                            ...
                            <tr class="color-black last-row-in-cart">
                                <th colspan="4" class="text-right">@lang('cart.full_price')</th>
                                <th class="full-price">{{ $currencySymbol }}<span
                                            id="totalPrice">{{ $cart->totalPrice()->format() }}</span></th>
                            </tr>

                            <tr>
                                <td colspan="5" class="text-center">
                                    <a href="{{ route('buy') }}" class="my-btn btn-black">@lang('button.checkout')</a>
                                </td>
                            </tr>
                            </tbody>
                        </table>
            ...
            @else
                ...
                <h5 class="color-black text-center">Но вы можете заполнить форму ниже...</h5>
            @endif
        </div>
        <div class="row">
            <div class="col-sm-10 col-sm-offset-1">
                <hr>
                @include("partials.cart.more")
            </div>
        </div>
    </div>
@endsection