...
<div class="form-group{{ $errors->has('customer_skype') ? ' has-error' : '' }}">
    <div class="row">
        <div class="col-sm-3 text-right">
            <label for="skype" class="control-label">@lang('form.skype')</label>
        </div>
        <div class="col-sm-9">
            <div class="skype">
                <input type="text" name="customer_skype" id="skype" class="form-control" required  value="{{ Auth::user()->skype ?? old('customer_skype') }}">
            </div>
            @if ($errors->has('customer_skype'))
                <span class="help-block">
                    <strong>{{ $errors->first('customer_skype') }}</strong>
                </span>
            @endif
        </div>
    </div>
